...
def _cmd_answer(target: str, answers: tuple[str, ...]) -> int:
    ...
    if not worker_is_alive(layout.session_dir):
        print(
            f"REFUSING: session {layout.session_id} is not running; only a live run"
            " holds a question open.",
            file=sys.stderr,
        )
        return 2
    prompt = _open_question(layout.session_dir)
    if prompt is None:
        print(f"REFUSING: {layout.session_id} is not waiting on a question.", file=sys.stderr)
        return 2
    if not answers:

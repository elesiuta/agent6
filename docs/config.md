...
| `verify_timeout_s` | `600.0` | Seconds one `verify_command` or `metric.command` call may take before it is killed and counted as failed. A model-chosen `run_command` is not bounded (see `command_checkin_s`). |
...
| `contract_examples` | `false` | Before the first model turn, one worker-model call derives the expected behaviour from the task as concrete input -> output examples, appended to the task as a <contract> block the worker verifies against the code. Off by default: an experiment knob, measured on SWE-rebench before any default moves. |

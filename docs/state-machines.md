...
- **No connections/secrets, no sandbox policy, no presets, no MCP servers, no host hooks**
    - `[config.providers.*]`, `[config.sandbox.*]`, `[config.presets.*]`, `[config.mcp.*]`, `git.run_repo_hooks`, `git.run_repo_filters`, `machine.notify`, `notify.on_complete`: each a load-time error
